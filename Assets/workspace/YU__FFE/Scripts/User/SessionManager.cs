namespace workspace.YU__FFE.Scripts.User {
    ...
    public static class SessionManager {
        ...
        // userSessions -> userSession 하나로 변경. 세션은 하나만 유지
        ...
        // ========== 세션 추가 및 저장 ==========
        public static void SetSessionData(string userId, string nickname, int profileNum,
                                      int coins, int grade, int rankPoint, int winCount, int loseCount) {
            _userSessionData = new UserSessionData(nickname,
                profileNum,
                coins,
                grade,
                rankPoint,
        ...
        private static void SaveUserSession() {
            string json = JsonUtility.ToJson(_userSessionData);
            PlayerPrefs.SetString(_userSessionData.Nickname, json);
